...
# runs stateful apps (home-ops live manifests, e.g. bazarr/atuin):
...
#   - config mounted read-only (the image ships none)
# Asserts /healthz and /readyz answer 200, the DB lands in /data, and the
# logs are free of errors.
set -euo pipefail

IMAGE="${IMAGE:-costanza:smoke}"
NAME="costanza-k8s-smoke-$$"
PORT="${PORT:-18143}"
...
check "sqlite db created in mounted /data" \
  docker exec "$NAME" test -f /data/costanza.db
check "clean logs (no errors/tracebacks)" \
  sh -c "! docker logs '$NAME' 2>&1 | grep -Ei 'traceback|\"level\": \"error\"'"
